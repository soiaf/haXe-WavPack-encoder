...
class CPPWvEncode
{
    public static function main()
    {
        ...
        var usage0:String= "";
        var usage1:String= " Usage:   cpp wavpack.n [-options] infile.wav outfile.wv [outfile.wvc]";
        ...
        while (arg_idx < cpp.Sys.args().length)
        {
            ...
            {
                if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-c") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-C") )
                {
                    if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-cc") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-CC"))
                    {
                ...
                else if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-f") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-F"))
                {
                    config.flags = haxe.Int32.or(config.flags, haxe.Int32.ofInt(Defines.CONFIG_FAST_FLAG));
                }
                else if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-h") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-H"))
                {
                    if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-hh") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-HH"))
                    {
                ...
                else if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-k") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-K"))
                {
                    var passedInt:Int= 0;

                    if (cpp.Sys.args()[arg_idx].length > 2)
                    {
                        try
                        {
                            var substring:String= cpp.Sys.args()[arg_idx].substr(2);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            passedInt = Std.parseInt(cpp.Sys.args()[arg_idx]);
                        }
                        catch (err: Dynamic)
                        {
                        }
                ...
                else if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-b") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-B"))
                {
                    var passedDouble:Float= 0;
                    config.flags = haxe.Int32.or(config.flags, haxe.Int32.ofInt(Defines.CONFIG_HYBRID_FLAG));

                    if (cpp.Sys.args()[arg_idx].length > 2)    // handle the case where the string is passed in form -b0 (number beside b)
                    {
                        try
                        {
                            var substring:String= cpp.Sys.args()[arg_idx].substr(2);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            passedDouble = Std.parseFloat(cpp.Sys.args()[arg_idx]);
                            config.bitrate = Math.floor((passedDouble * 256.0));
                        }
                        catch (err: Dynamic)
                        {
                    ...
                    {
                        cpp.Lib.println("hybrid spec must be 2.0 to 16.0!");
                        ++error_count;
                    }
                }
                else if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-j") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-J"))
                {
                    var passedInt:Int= 2;

                    if (cpp.Sys.args()[arg_idx].length > 2) // handle the case where the string is passed in form -j0 (number beside j)
                    {
                        try
                        {
                            var substring:String= cpp.Sys.args()[arg_idx].substr(2);
                    ...
                    {
                        cpp.Lib.println("-j0 or -j1 only!");
                        ++error_count;
                    }
                }
                else if (StringTools.startsWith(cpp.Sys.args()[arg_idx], "-s") || StringTools.startsWith(cpp.Sys.args()[arg_idx], "-S"))
                {
                    var passedDouble:Float= 0; // noise shaping off

                    if (cpp.Sys.args()[arg_idx].length > 2) // handle the case where the string is passed in form -s0 (number beside s)
                    {
                        try
                        {
                            var substring:String= cpp.Sys.args()[arg_idx].substr(2);
                    ...
                    {
                        cpp.Lib.println("-s-1.00 to -s1.00 only!");
                        ++error_count;
                    }
                }
                else
                {
                    cpp.Lib.println("illegal option: " + cpp.Sys.args()[arg_idx]);
                    ++error_count;
                }
            }
            else if (infilename.length == 0)
            {
                infilename = cpp.Sys.args()[arg_idx];
            }
            else if (outfilename.length == 0)
            {
                outfilename = cpp.Sys.args()[arg_idx];
            }
            else if (out2filename.length == 0)
            {
                out2filename = cpp.Sys.args()[arg_idx];
            }
            else
            {
        ...
        {
            cpp.Lib.println("high and fast modes are mutually exclusive!");
            ++error_count;
        }
        ...
        if(haxe.Int32.compare(zeroCheck, haxe.Int32.and(config.flags, haxe.Int32.ofInt(Defines.CONFIG_HYBRID_FLAG))) != 0 )
        {
            ...
            {
                cpp.Lib.println("need name for correction file!");
                ++error_count;
            }
        }
        else
        {
            ...
            {
                cpp.Lib.println("-s and -c options are for hybrid mode (-b) only!");
                ++error_count;
            }
        }
        ...
        {
            cpp.Lib.println("third filename specified without -c option!");
            ++error_count;
        }

        if (error_count == 0)
        {
            cpp.Lib.println(sign_on1);
            cpp.Lib.println(sign_on2);
        }
        else
        {
            cpp.Sys.exit(1);
        ...
        {
            cpp.Lib.println(usage0);
            cpp.Lib.println(usage1);
            cpp.Lib.println(usage2);
            cpp.Lib.println(usage3);
            cpp.Lib.println(usage4);
            cpp.Lib.println(usage5);
            cpp.Lib.println(usage6);
            cpp.Lib.println(usage7);
            cpp.Lib.println(usage8);
            cpp.Lib.println(usage9);
            cpp.Lib.println(usage10);
            cpp.Lib.println(usage11);
        cpp.Lib.println(usage12);
        ...
        cpp.Lib.println(end - start + " milli seconds to process WavPack file in main loop");


        if (result > 0)
        {
            cpp.Lib.println("error occured!");
            ++error_count;
        }
    }
    ...
    static function pack_file(infilename:String, outfilename:String, out2filename:String,
        ...
        var din = cpp.io.File.read(infilename,true);
        
        var wv_file = cpp.io.File.write(outfilename,true);
        ...
        {
            cpp.Lib.println(infilename + " is not a valid .WAV file!");
        ...
        while (true)
        {
                    ...
                    (chunk_header[3] == 32))
            {
                ...
                {
                    cpp.Lib.println(infilename + " is an unsupported .WAV format!");
            ...
            { // just skip over unknown chunks
                ...
                {
                    cpp.Lib.println("error occurred in skipping bytes");
        ...
        {
            wvc_file = cpp.io.File.write(out2filename,true);
        ...
        {
            cpp.Lib.println(WavPackUtils.WavpackGetErrorMessage(wpc));
        ...
        {
            cpp.Lib.println("couldn't read all samples, file may be corrupt!!");
        ...
        {
            cpp.Lib.println("Can't close WavPack file!");
